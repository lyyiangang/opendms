...
namespace opendms
{
    ...
    bool Fatigue::Process(const Frame &frame, const FaceData &face_data)
    {
        ...
        {
            // yawn?
            float yawn_width = cv::norm(landmarks.row(48) - landmarks.row(54));
            float yawn_height = cv::norm(landmarks.row(66) - landmarks.row(62));
            if (yawn_height / yawn_width > 0.4)
            {
                yawn = true;
            }
            // close eye?
            for (int ii = 0; ii < 2; ++ii)
            {
                ...
                float width = cv::norm(landmarks.row(corner_idx[0]) - landmarks.row(corner_idx[1]));
                float height = cv::norm((landmarks.row(up_idx[0]) + landmarks.row(up_idx[1])) / 2,
