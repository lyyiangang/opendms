...
def gen_cmd(dali_root_dir, file_list, process_includes=False):
    ...
    if not file_list:
        return ["true"]
    cmd = ["python",
           os.path.join(dali_root_dir, "3rdparty", "cpplint.py"),
           "--quiet",
           "--linelength=100",
...
def main(dali_root_dir, n_subproc=1, file_list=None):
    cc_files = gather_files(
        os.path.join(dali_root_dir, "src"),
        ["*.cc", "*.h", "*.cpp", "*.hpp", "*.cxx", "*.hxx"] if file_list is None else file_list,
        negative_filters)

    inc_files = gather_files(
        os.path.join(dali_root_dir, "include"),
        ["*.h", "*.inc", "*.inl", "*.hxx", "*.hpp"] if file_list is None else file_list,
        negative_filters)
